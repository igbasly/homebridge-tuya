...
const TemperatureAndHumidityAccessory = require('./lib/TemperatureAndHumidityAccessory');
...
const CLASS_DEF = {
    ...
    temperatureandhumiditysensor: TemperatureAndHumidityAccessory
};
...
class TuyaLan {
    ...
    discoverDevices() {
        ...
        this.config.devices.forEach(device => {
            ...
            // Now its generating ascii local keys
            if (!/^[\x00-\x7F]+$/i.test(device.key)) return this.log.error('%s, key for %s (%s), is not a valid key.', device.key.replace(/.{4}$/, '****'), device.name || 'unnamed device', device.id);
